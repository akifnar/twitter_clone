...
// Tweetleri hafızada tutmak için fonksiyon dışına aldık.
// Böylece yeni tweet attığında sayfayı yenilesen de (server kapanana kadar) gitmez.
const tweets = [
  {
    id: '1001',
...
export async function POST(request) {
  try {
    // Frontend'den gelen veriyi al (Örn: { text: "Merhaba dünya" })
    const body = await request.json()

    // Yeni bir tweet objesi oluştur
    const newTweet = {
      id: Date.now().toString(), // Benzersiz ID
      text: body.text, // Senin yazdığın metin
      created_at: new Date().toISOString(), // Şu anki tarih
      public_metrics: {
        // Beğeni sayıları sıfırdan başlar
        like_count: 0,
        retweet_count: 0,
        reply_count: 0,
        quote_count: 0
    ...
    // Başarılı cevabı dön
    return NextResponse.json({ success: true, data: newTweet })
  } catch (error) {
    return NextResponse.json({ error: 'Tweet atılamadı' }, { status: 500 })
  }
}
